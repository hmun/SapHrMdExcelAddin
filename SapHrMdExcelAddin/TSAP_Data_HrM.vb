...
Public Class TSAP_Data_HrM
    ...
    Public Sub New(ByRef pPar As SAPCommon.TStr, ByRef pIntPar As SAPCommon.TStr, ByRef pSAPHrMaster As SAPHrMaster, pFunction As String)
        aPar = pPar
        aIntPar = pIntPar
        aFunction = pFunction
        ...
        ' get Metadata
        If pFunction = "SaveReplMulti" Then
            pSAPHrMaster.getMeta_SaveReplMulti(aParamDic, aStrucDic)
        End If
    End Sub
        ...
        For Each aKvb In aPar.getData()
            ...
            For s As Integer = 0 To aStrucName.Length - 1
                ...
                If valid_StructureX_Field(aNewTStrRec, aLen) Then
                    If aLen = 1 Then
                        aNewHdrRec.setValues(aNewTStrRec.Strucname & "X-" & aNewTStrRec.Fieldname, "X", "", "")
                    Else
                        aNewHdrRec.setValues(aNewTStrRec.Strucname & "X-" & aNewTStrRec.Fieldname, aNewTStrRec.Value, aNewTStrRec.Currency, aNewTStrRec.Format, pEmptyChar:="")
                    End If
                End If
                If valid_Table_Field(aNewTStrRec) Then
                    aDataDic.addValue("1", aNewTStrRec)
                End If
            Next
        Next
    ...
    Public Function valid_StructureX_Field(pTStrRec As SAPCommon.TStrRec, ByRef pLen As Integer) As Boolean
        ...
        aStrucName = Split(pTStrRec.Strucname, "+")
        For s As Integer = 0 To aStrucName.Length - 1
            If aStrucDic.ContainsKey("S|" & aStrucName(s) & "X") Then
                valid_StructureX_Field = isInStructure(pTStrRec.Fieldname, aStrucDic("S|" & aStrucName(s) & "X"), pLen)
            End If
        Next
    End Function
    ...
    Public Function valid_TableX_Field(pTStrRec As SAPCommon.TStrRec, ByRef pLen As Integer) As Boolean
        ...
        aStrucName = Split(pTStrRec.Strucname, "+")
        For s As Integer = 0 To aStrucName.Length - 1
            If aStrucDic.ContainsKey("T|" & aStrucName(s) & "X") Then
                valid_TableX_Field = isInStructure(pTStrRec.Fieldname, aStrucDic("T|" & aStrucName(s) & "X"), pLen)
            End If
        Next
    End Function
